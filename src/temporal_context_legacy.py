# src/temporal_context_legacy.py
import torch
...
class TemporalContextEncoder(nn.Module):
    """
    Legacy LSTM-based encoder for loading old models.
    This version uses LSTM to maintain compatibility with existing saved models.
    """
    ...
    def __init__(self, config):
        ...
        # Bidirectional LSTM for processing sequences (legacy version)
        self.lstm = nn.LSTM(
        ...
        self.trend_analyzer = nn.Sequential(
            nn.Linear(2 * self.temporal_hidden_dim, config['trend_hidden_dim']),
            nn.ReLU(),
            nn.Dropout(config['dropout']),
    ...
    def forward(self, sequence_tensor):
        ...
        # Process through LSTM - set to eval mode for inference to avoid training mode issues
        self.lstm.eval()
        with torch.no_grad():
        ...
        trend_features = self.trend_analyzer(context_raw)  # [batch, trend_features]
